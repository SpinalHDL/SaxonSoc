...
class SaxonSocOnChipRam extends Generator {
...
object SaxonSocOnChipRam {
  def main(args: Array[String]): Unit = {
    SpinalRtlConfig.generateVerilog(new SaxonSocOnChipRam().defaultSetting().toComponent())
  }
}


object SaxonSocOnChipRamSynthesisBench {
  def main(args: Array[String]) {
    val briey = new Rtl {
      ...
      SpinalVerilog({
        val soc = new SaxonSocOnChipRam().defaultSetting().toComponent().setDefinitionName(getRtlPath().split("\\.").head)
