...
import vexriscv.plugin.{BranchPlugin, CfuParameter, CfuPlugin, CfuTest, CsrAccess, CsrPlugin, CsrPluginConfig, DBusSimplePlugin, DecoderSimplePlugin, FullBarrelShifterPlugin, HazardSimplePlugin, IBusSimplePlugin, IntAluPlugin, LightShifterPlugin, RegFilePlugin, SrcPlugin, YamlPlugin}
...
object Ice40up5kbevnNoXipSystem{
  def default(g : Ice40up5kbevnNoXipSystem, clockCtrl : ClockDomainGenerator) = g {
    ...
    cpu produce new Area{
      val cfuTest = CfuTest()
      cpu.config.plugins.foreach{
        case cfuPlugin : CfuPlugin => cfuTest.io.bus << cfuPlugin.bus
        case _ =>
      }
    }
...
object Ice40up5kbevnNoXipSystemSim {
  ...
  def main(args: Array[String]): Unit = {
    ...
    simConfig.withWave
    simConfig.addRtl("hardware/netlist/up5k_cells_sim.v").compile(new Ice40up5kbevnNoXipSystem(){
      val clockCtrl = ClockDomainGenerator()
